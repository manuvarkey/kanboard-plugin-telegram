...
<p class="alert alert-info">
  ...
  <?= t('Go to ') ?> <a href="<?= $this->url->href('ConfigController', 'integrations') ?>">
    <?= t('Settings > Integrations > Telegram') ?> </a> <?= t(' and fill the form:') ?> <br />
...
<p>
  ...
  <?= t('1. Send the message %s to', $random) ?> <a href="<?= $bot_url ?>" target="_blank"
    rel="noreferrer"><?= $bot_url ?></a><br />
...
<div class="panel">
  ...
  <?= $this->form->label(t('Topic id of group chat with bot (message_thread_id)'), 'telegram_group_msg_tid') ?>
  <?= $this->form->text('telegram_group_msg_tid', $values, array()) ?>
